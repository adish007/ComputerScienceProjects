...
public class AVL<Key extends Comparable<Key>> {
	...
	private void counts(Node parent, SeparateChainingHashST<Integer,Integer> ht) {
		...
		Integer HTD = ht.get(heightdiff);
		
		if(HTD == null) {
			ht.put(heightdiff, 1);
		}
		else {
			ht.put(heightdiff, HTD+1);
		}
	...
	private int minDepth(Node parent, int currentDepth, int lowestSoFar) {
		...
		lowestSoFar = Math.min(minDepth(parent.left, currentDepth+1,lowestSoFar),minDepth(parent.right, currentDepth+1,lowestSoFar));
